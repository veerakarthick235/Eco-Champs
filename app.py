...
try:
    ...
    # Explicitly select the database. This is more robust.
...
# --- Database Collections (defined from the direct 'db' object) ---
if db is not None:
    ...
    # Set to None so the app will crash if the DB is not available
...
# (The rest of your app.py routes are perfect and need no changes)
# ... all other routes from /login to the end ...
...
def login():
    ...
    if request.method == 'POST':
        ...
        if user_data and bcrypt.check_password_hash(user_data['password'], password):
            ...
            if last_login_date is None or last_login_date < today: # First login or login on a new day
...
def challenges_page():
    ...
    # Create a map of challenge_id to submission status for easy lookup
...
def leaderboard():
    # City leaderboard
    city_pipeline = [
        {"$group": {"_id": "$city", "total_points": {"$sum": "$points"}}},
        {"$sort": {"total_points": -1}},
        {"$limit": 10}
    ]
    ...
    # School leaderboard
    school_pipeline = [
        {"$group": {"_id": "$school", "total_points": {"$sum": "$points"}}},
        {"$sort": {"total_points": -1}},
        {"$limit": 10}
    ]
...
def admin_dashboard():
    pending_submissions = list(submissions_collection.aggregate([
        ...
        {'$unwind': '$user_info'},
        {'$lookup': {'from': 'challenges', 'localField': 'challenge_id', 'foreignField': '_id', 'as': 'challenge_info'}},
...
def handle_submission(submission_id, action):
    ...
    if action == 'approve':
        ...
        new_points = user_data.get('points', 0) + challenge['points']
        new_badges = calculate_badges(new_points)

        users_collection.update_one(
            {'_id': submission['user_id']},
            {'$inc': {'points': challenge['points']}, '$set': {'badges': new_badges}}
        )
...
def submit_quiz():
    ...
    correct_answers = sum(1 for i, q in enumerate(quiz_questions) if user_answers[i] == q['correct_answer'])
    ...
    users_collection.update_one(
        ...
        {'$inc': {'points': points_earned}, '$set': {'badges': new_badges}}
    )
